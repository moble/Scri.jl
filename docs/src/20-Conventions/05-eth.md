...
\cong \mathrm{SU}(2)``, the group of unit quaternions.  Then we can
easily define ``\eth`` in those terms.  Start with the
right-derivative operator with respect to the generator ``𝔤`` acting
on a function ``f`` and evaluated at ``Q``:
