"""
    aberration(RRₚᵢ, v⃗, εᴵ=+1)

Transform from the rotor `RRₚᵢ` in the boosted frame to the corresponding rotor in the
rest frame, given the boost velocity vector `v⃗`, with `εᴵ = ±1` selecting future or past
null infinity — see [Future and past null infinity](@ref scri_pm_conventions).

The result is the ``K`` factor of the Iwasawa ``KAN`` decomposition of the full Lorentz
transformation ``Λ =`` `Boost(εᴵ*v⃗) * RRₚᵢ`: it points along the same null direction for
the rest observer as ``Λ`` produces for the boosted observer, and carries the tangent-frame
...
the idempotent ``u₊ = (1+𝐭𝐳)/2`` gives ``ℂℜ(Λu₊) = ½e^{φₐ/2}𝐑_K``, which normalizes
to ``𝐑_K``.  See the "Reversing the order" section of [The Lorentz Group](@ref) page for
why ``K`` is the right object, and [the `Quaternionic` documentation](@extref Quaternionic
:std:label:`iwasawa-kan`) for the extraction.

The sign `εᴵ` enters only through the boost velocity.  The ``u₊`` extraction is tied to
the null vector ``ℓ ∝ (1, +n̂)`` after the pixel rotor carries ``𝐳 → n̂``, which matches
the ℐ⁺ labeling directly.  At ℐ⁻ the labeling is antipodal — pixel ``n̂`` labels the null
vector ``(1, −n̂)`` — and conjugating by spatial parity turns ``B(v⃗)`` acting on ``(1,
−n̂)`` into ``B(−v⃗)`` acting on the label ``(1, +n̂)``, while leaving the
parity-invariant rotation factor ``K`` unchanged.  Thus both cases reduce to
`Boost(εᴵ*v⃗)` with the same extraction.
...
"""
function aberration(RRₚᵢ::Rotor{T}, v⃗, εᴵ::Int=+1) where {T}
    u₊ = (1 + im * 𝐤) / T(2)
    B = Boost(εᴵ * v⃗)
    Λ = B * RRₚᵢ
    λ = Λ * u₊
...
@testitem "aberration: identity at β=0" tags = [:unit, :fast] setup = [AberrationSetup] begin
    ...
    # RRₚᵢ itself, for either sign of εᴵ.
    for T ∈ FloatTypes, εᴵ ∈ (-1, +1)
        let π = T(π)
            ...
            for R ∈ [
                ...
                @test components(aberration(R, v⃗_zero, εᴵ)) ≈ components(R) atol = 4eps(T)
            end
        end
    end
end
...
@testitem "aberration: pole invariance — no tangent rotation along boost axis" tags = [
    ...
    # This holds for both εᴵ = ±1 since flipping v⃗ preserves (anti-)alignment.
    for T ∈ FloatTypes, εᴵ ∈ (-1, +1)
        ϵ = eps(T)
        ...
        for β ∈ T.([0.1, 0.5, 0.9])
            v⃗ = QuatVec(0, 0, β)
            @test components(aberration(R_north, v⃗, εᴵ)) ≈ components(R_north) atol=4ϵ
            @test components(aberration(R_south, v⃗, εᴵ)) ≈ components(R_south) atol=4ϵ
        end
    end
end
